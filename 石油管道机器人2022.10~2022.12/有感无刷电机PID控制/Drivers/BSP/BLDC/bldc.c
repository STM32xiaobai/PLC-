...
{       
    ...
    SHUTDOWN_PIN_GPIO_CLK_ENABLE();    //PF10 -- SHDN引脚
    ...
    atim_timx_oc_chy_init(arr,  psc);       /* 定时器初始化 */
		btim_timx_int_init(1000 - 1 , 72 - 1);
}
...
/**
  * @brief  霍尔传感器接口初始化  PC6，7，8分别为霍尔UVW
  * @param  无
  * @retval 无
  */
...
uint32_t hallsensor_get_state(uint8_t motor_id)
{
    ...
    state  = 0;
		...
		if(HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_6) != GPIO_PIN_RESET)  /* 霍尔传感器状态获取 */
		{
			state |= 0x01;
		}
		if(HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_7) != GPIO_PIN_RESET)  /* 霍尔传感器状态获取 */
		{
			state |= 0x02;
		}
		if(HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_8) != GPIO_PIN_RESET)  /* 霍尔传感器状态获取 */
		{
			state |= 0x04;
