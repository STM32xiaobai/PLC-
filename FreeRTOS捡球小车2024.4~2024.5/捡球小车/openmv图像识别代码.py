...
#            tx = b"0x6A" + bytes([aver[0], aver[1], aver[2] % 256, aver[2] // 256])
#            uart_buf = [0x6A, aver[0], aver[1], aver[2] % 256, aver[2] // 256, 0x6D]
            uart_buf = bytearray([0x6A, aver[0], aver[1], aver[2] % 256, aver[2] // 256, 0x6D])
            print(aver[0], aver[1], aver[2] % 256, aver[2] // 256)
        else:
            img.draw_string(0, 60, str(len(res)), (255, 255, 0))
#            tx = b"0x6B\x00\x00\x00\x00"
            uart_buf = bytearray([0x6B, 0, 0, 0, 0, 0x6D])
    else:
        img.draw_string(0, 60, "Not found!", (255, 255, 0))
        res = []
#        tx = b"0x6C\x00\x00\x00\x00"
        uart_buf = bytearray([0x6B, 0, 0, 0, 0, 0x6D])
